...
function construct(e::ProjectionMatrices{<:HomographyMatrices, T₂ , T₃}, ℋ::Tuple{AbstractArray, Vararg{AbstractArray}}) where {T₂ <: FactorisationAlgorithm, T₃ <: TotalViews }
    𝛈 = deconstruct(LatentVariables(HomographyMatrices()), ℋ)
    𝐚  = @view 𝛈[1:9]
    𝐀 = reshape(𝐚, (3,3))
    𝐛 = @view 𝛈[10:12]
    𝐏₁ = SMatrix{3,4}(1.0I)
